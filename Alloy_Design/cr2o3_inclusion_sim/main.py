...
main.py
-------
Cr2O3 inclusion shape relaxation — Allen-Cahn phase-field simulation.
 
Two run modes (set RUN_MODE in CONFIG):
  "single"   — simulate one inclusion, one cooling rate.
  "sweep"    — grid of inclusion sizes × cooling rates. Produces all plots.
"""
...
CONFIG = {
    ...
    # delta_T = T_melt - T_solidus = 2100 - 1280 = 820 degC
    "delta_T":    820.0,
...
def _style_ax(ax):
    ...
    for side in ["bottom", "left"]:
        ax.spines[side].set_edgecolor(SPINE_COL)
...
def build_inclusion(cfg, radius_px, grid_size=None, seed=None):
    N = grid_size or cfg["grid_size"]
    s = seed if seed is not None else cfg["seed"]
    return irregular_polygon(
        grid_shape=(N, N), mean_radius=radius_px,
...
def run_sweep(cfg):
    ...
    for cr in cooling_rates:
        n_steps, t_dim, t_phys = get_steps(cfg, cr)
        save_every = max(1, n_steps // cfg["n_snapshots"])
        print(f"\n--- CR = {cr} °C/s  |  t_phys = {t_phys:.1f}s  |  "
              f"n_steps = {n_steps}  |  save_every = {save_every} ---")
 
        for r_um in radii_um:
            r_px = r_um / dx_um
            if r_px < 8:
                sweep_data[cr][r_um] = None
                print(f"  R={r_um}µm: too small, skip")
                continue
            ...
            ci = _circularity(phi_init)
            cf = _circularity(phi_f)
            sweep_data[cr][r_um] = {
                "circ_init":   ci,
                "circ_final":  cf,
                "circ_series": metrics["circularity"],
                "step_series": metrics["steps"],
                "t_phys":      t_phys,
                "n_steps":     n_steps,
                "phi_init":    phi_init,
                "phi_final":   phi_f,
...
def plot_circularity_vs_radius(sweep_data, cfg, out):
    ...
    for r_um in radii_um:
        ...
        if d:
            init_r.append(r_um); init_c.append(d["circ_init"])
    ax.plot(init_r, init_c, color="#777777", lw=1.5, ls="--",
            label="Initial shape", zorder=2)
    ...
    for cr in cooling_rates:
        ...
        for r_um in radii_um:
            ...
            if d:
                rs.append(r_um); cfs.append(d["circ_final"])
        ax.plot(rs, cfs, color=col, lw=2, marker="o", ms=6,
                label=f"{cr} °C/s", zorder=3)
    ...
    fig.text(0.5, -0.04,
             f"M = {cfg['mobility']:.0e} m³J⁻¹s⁻¹  |  "
             f"σ = {cfg['sigma']} Jm⁻²  |  δT = {cfg['delta_T']:.0f} °C",
    ...
    plt.tight_layout()
    _savefig(fig, "circularity_vs_radius.png", out)
 
 
# ===========================================================================
# Plot 2 — circularity vs step, by cooling rate
# ===========================================================================
...
def plot_circ_vs_step_by_cr(sweep_data, cfg, fixed_cr, out):
    ...
    for cr in cooling_rates:
        ...
        if not d:
            continue
        col    = CR_COLOURS.get(cr, "#ffffff")
        steps  = np.array(d["step_series"])
        circs  = np.array(d["circ_series"])
        t_phys = d["t_phys"]
        n_steps = d["n_steps"]
        times  = steps / max(n_steps, 1) * t_phys
    ...
    for ax, xl in zip(axes, ["Simulation step", "Physical time  (s)"]):
        ...
        ax.legend(fontsize=8, facecolor="#333", labelcolor=TEXT_COL, framealpha=0.85)
    axes[0].set_title(f"Circularity evolution — R = {mid_r} µm  (by cooling rate)",
                      color=TITLE_COL, fontsize=11, pad=6)
    axes[1].set_title(f"Circularity vs physical time — R = {mid_r} µm",
                      color=TITLE_COL, fontsize=11, pad=6)
    fig.suptitle("Shape relaxation dynamics — cooling rate dependence",
                 color="#eee", fontsize=12, y=1.02)
    plt.tight_layout()
    _savefig(fig, "circ_vs_step_by_cooling_rate.png", out)
 
 
# ===========================================================================
# Plot 3 — circularity vs step, by radius
# ===========================================================================
...
def plot_circ_vs_step_by_radius(sweep_data, cfg, out):
    ...
    for ax, xl in zip(axes, ["Simulation step", "Physical time  (s)"]):
        ...
        ax.set_ylabel("Circularity", color=TICK_COL, fontsize=10)
        ax.set_xlabel(xl, color=TICK_COL, fontsize=10)
        ax.legend(fontsize=8, facecolor="#333", labelcolor=TEXT_COL,
                  framealpha=0.85, title="Radius", title_fontsize=8)
    axes[0].set_title(f"Circularity evolution — CR = {fixed_cr} °C/s  (by radius)",
                      color=TITLE_COL, fontsize=11, pad=6)
    axes[1].set_title(f"Circularity vs physical time — CR = {fixed_cr} °C/s",
                      color=TITLE_COL, fontsize=11, pad=6)
    fig.suptitle("Shape relaxation dynamics — inclusion size dependence",
                 color="#eee", fontsize=12, y=1.02)
    plt.tight_layout()
    _savefig(fig, "circ_vs_step_by_radius.png", out)
 
 
# ===========================================================================
# Plot 4 — shape comparison panels, one PNG per cooling rate
# ===========================================================================
 
def plot_shape_comparisons(sweep_data, cfg, dx_um, out):
    from matplotlib.colors import LinearSegmentedColormap
    oxide_cmap = LinearSegmentedColormap.from_list(
        "oxide", ["#f5f0e8", "#8b6914", "#3d2b00"])
    radii_um      = cfg["sweep_radii_um"]
    cooling_rates = cfg["sweep_cooling_rates"]
 
    for cr in cooling_rates:
        valid = [(r, sweep_data[cr][r]) for r in radii_um
                 if sweep_data[cr].get(r)]
        if not valid:
            continue
        n = len(valid)
        ...
        for row, (r_um, d) in enumerate(valid):
            for col_idx, (phi, label) in enumerate([
                ...
                ax     = axes[row][col_idx]
                grid   = d["grid"]
                extent = [0, grid * dx_um, 0, grid * dx_um]
                ax.imshow(phi, origin="lower", cmap=oxide_cmap,
                ...
                ax.set_title(f"R = {r_um} µm — {label}",
                             color=TITLE_COL, fontsize=9, pad=4)
                ax.set_xlabel("µm", color=TICK_COL, fontsize=8)
                ax.set_ylabel("µm", color=TICK_COL, fontsize=8)
 
        fig.suptitle(
            f"Inclusion shape relaxation — cooling rate {cr} °C/s\n"
            f"δT = {cfg['delta_T']:.0f} °C  |  "
            f"t_solid = {cfg['delta_T']/cr:.1f} s  |  "
            f"M = {cfg['mobility']:.0e} m³J⁻¹s⁻¹",
            color="#eee", fontsize=11, y=1.01)
        plt.tight_layout()
        _savefig(fig, f"shape_comparison_CR{int(cr):03d}.png", out)
